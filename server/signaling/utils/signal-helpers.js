...
async function ClientJoinsMeeting(ws, payload, senderId) {
    ...
    console.info(`Client ${ws.clientId} joined meeting ${meetingID}.`);
...
function startHeartbeat(clients) {
  const heartbeatInterval = setInterval(async () => {
      try {
          ...
          console.log(`Signaling server ${signalingServerId} has tracked a heartbeat at ${Date.now()}`);
          // Update metrics in Redis
          await redis.hset(`signaling:${signalingServerId}:metrics`, 
...
module.exports = { RegisterClientSfu, ClientJoinsMeeting, ClientLeavesMeeting, WebRTCHandler, SfuSignalToClient, WebSocketDisconnectClient, WebSocketDisconnectSfu, startHeartbeat };