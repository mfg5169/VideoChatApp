...
func initLogger() {
	...
	level := INFO
	if envLevel := os.Getenv("SFU_LOG_LEVEL"); envLevel != "" {
		switch envLevel {
		case "DEBUG":
			level = DEBUG
		case "INFO":
	...
	sfuLogger = NewLogger(sfuID, level)
	sfuState = NewSFUState(sfuID)

	sfuLogger.Info("LOGGER", "SFU logger initialized", map[string]interface{}{
		"level": level.String(),
		"sfuID": sfuID,
	})
}
